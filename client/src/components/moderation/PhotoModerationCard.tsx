    ...
    <>
      <Card className="col-span-1 bg-white rounded-lg shadow divide-y divide-gray-200">
        ...
        <div className="p-4">
          ...
          {/* For approved photos - show archive button */}
          {photo.status === "approved" && (
            <Button
          ...
          {/* For rejected photos - only show approve button */}
          {photo.status === "rejected" && (
            <Button
              onClick={() => handleModerate("approve")}
              disabled={moderateMutation.isPending}
              variant="default"
              className="inline-flex items-center px-3 py-2 border border-transparent shadow-sm text-sm leading-4 font-medium rounded-md text-white bg-green-600 hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-500 w-full justify-center"
            >
              <svg className="-ml-0.5 mr-2 h-4 w-4" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M5 13l4 4L19 7" />
              </svg>
          ...
          {/* For archived photos - only show restore to approved button */}
          {photo.status === "archived" && (
            <Button
              onClick={() => handleModerate("approve")}
              disabled={moderateMutation.isPending}
      ...
      {/* Confirmation Dialog for approving rejected posts */}
      <AlertDialog open={confirmDialogOpen} onOpenChange={setConfirmDialogOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>Are you sure?</AlertDialogTitle>
            <AlertDialogDescription>
              Are you sure you want to mark this post as approved?
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel>No</AlertDialogCancel>
            <AlertDialogAction onClick={handleConfirmApprove}>Yes</AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>
    </>
